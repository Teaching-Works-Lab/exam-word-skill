...
创建一个可自动发现、也可通过 `$exam-word` 显式调用的个人 Skill，稳定处理不同学院和课程试卷的 Word 对比、格式检查、模板套用、PDF 转标准 Word 和结果验证。
...
- 发布前验证仓库可见性符合用户要求，且提交清单不含原始试卷和临时文件。
